...
def convert_obsidian_callouts(text):
    """Convert Obsidian callouts to MkDocs format."""
...
def find_target_file(docs_path, link_target):
    ...
    Args:
        ...
        link_target: The target from the link (e.g., "Lexicon/Keywords/Keyword_Drone")
    
    Returns:
        Path object if found, None otherwise
    """
    # Clean up the link target - remove any trailing backslashes or whitespace
    link_target = link_target.strip().rstrip('\\').rstrip('/')
...
def convert_obsidian_links(text, source_file_path, docs_path):
    ...
    # Pattern 1: Handle bracketed links (both escaped and unescaped)
    # Matches: \[[[path|display]]\] OR [[[path|display]]]
    def replace_bracketed_link(match):
        # Get the inner content, which is everything between [[[ and ]]]
        inner_link = match.group(1)
        
        # Clean up - remove any backslashes that might be in the content
        ...
        if target_file:
            ...
            # Return with brackets around the converted link
            return f"[[{display}]({rel_path})]"
        else:
            print(f"    WARNING: Could not find target for bracketed link: {path}")
            # Return original if we can't find the target
            return match.group(0)
    
    # This pattern matches [[[anything]]] with optional backslashes before/after brackets
    # The key is to capture everything between the triple brackets
    text = re.sub(r'\\?\[\\?\[\[([^\]]+?)\]\\?\]\\?\]', replace_bracketed_link, text)
    
    # Pattern 2: Handle regular Obsidian links
    # Matches: [[path|display]] or [[path]]
    # But NOT [[display](path)] which is already converted
    def replace_regular_link(match):
        content = match.group(1)
        
        # Skip if this looks like it's already been converted (contains markdown link syntax)
        if '](' in content:
            return match.group(0)
    ...
    # Match regular [[...]] links (not triple brackets, not already converted)
