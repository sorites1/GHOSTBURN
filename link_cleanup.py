...
def process_section(lines, in_table):
    ...
    for line in lines:
        ...
        for term, (term_type, term_filename) in all_terms.items():
            ...
            # Need to match the escaped brackets at start and end
            # Pattern: \[[[Type_Name with optional \|Name]]\]
            pattern = r'\\?\[\[\[' + re.escape(f'{term_type}_{term_filename}') + r'\\?\|' + re.escape(term) + r'\]\]\]\\?\]'
